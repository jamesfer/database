...
export const hashPartitionSerializableFacade: SerializableFacade<HashPartitionConfiguration> = {
  serialize(object: HashPartitionConfiguration): string {
    const nestedConfigSerializer: SerializableFacade<AllComponentConfigurations> | undefined = AllComponentsLookup[object.nestedConfig.NAME].FACADES[SERIALIZABLE_FACADE_FLAG];
    ...
    return JSON.stringify({
      ...
      nestedConfigName: object.nestedConfig.NAME,
      nestedConfig: nestedConfigSerializer.serialize(object.nestedConfig),
    });
  },
  deserialize(string: string): HashPartitionConfiguration | undefined {
    ...
    const nestedConfig = nestedConfigSerializer.deserialize(json.nestedConfig);
    assert(nestedConfig, 'Failed to deserialize nested config');
    assert(
      componentConfigurationImplements([SERIALIZABLE_FACADE_FLAG, EQUALS_FACADE_NAME, DISTRIBUTED_OPERATOR_FACADE_NAME], nestedConfig),
      'Nested config of a hash partition must implement the equals, serializable, and distributed operator facades',
    );

    return new HashPartitionConfiguration(json.partitionCount, nestedConfig);
  }
};
