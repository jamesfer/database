...
import { HashPartitionDetails, HashPartitionInternalConfiguration } from '../hash-partition-internal-configuration';
    ...
    return events$.pipe(
      ...
      concatMap(async ([config, nodes]) => {
        ...
        for (let i = 0; i < config.partitionsCount; i++) {
          ...
          if (!existingNestedConfig || !componentEquals(existingNestedConfig, config.nestedConfig)) {
