...
export function findHashPartition(value: BinaryLike, partitions: number): number {
  ...
  if (selectedPartition >= partitions) {
    throw new Error(`There is an error in the hashing algorithm. A partition for a value was greater than the total number of partitions. Value: ${value.toString()}, partitions: ${partitions}, selected partition: ${selectedPartition}`);
  }
