...
import { AnyRequest, unifiedRequestRouter } from '../src/routing/unified-request-router';
...
import { HashPartitionConfiguration } from '../src/components/hash-partition/hash-partition-configuration';
...
describe('database', () => {
  it('runs a simple scenario with the in memory links', async () => {
    ...
    const hashPartitionConfig = new HashPartitionConfiguration(5, new SimpleInMemoryKeyValueConfiguration());
    await rpcInterface.makeRequest({
      category: RequestCategory.MetadataTemporary,
      action: MetadataTemporaryAction.Put,
      targetNodeId: 'node0',
